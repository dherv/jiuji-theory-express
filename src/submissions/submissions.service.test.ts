...
test('findAll() should return all', async () => {
  const data = await submissionsService(mockRepository).findAll();
...
test('findOne() should return one', async () => {
  ...
  const data = await submissionsService(mockRepository).findOne(id);
  expect(mockRepository.findOne).toHaveBeenCalledWith(id);
  expect(data).toEqual({ data: 'mockRepository' });
});
test('create() should create one', async () => {
  const { body, user } = mockRequest;
  const data = await submissionsService(mockRepository).create(body, user);
...
test('update() should update one', async () => {
  ...
  const data = await submissionsService(mockRepository).update(body, id, user);
...
test('delete() should delete one', async () => {
  ...
  const data = await submissionsService(mockRepository).delete(id);
